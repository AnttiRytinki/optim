...
static int refinementCount;

static const int DirectionX[DIRECTION_COUNT] = {0, 1, 1, 1, 0, -1, -1, -1};

static const int DirectionY[DIRECTION_COUNT] = {-1, -1, 0, 1, 1, 1, 0, -1};
...
static double GetMaximumStep(void) {
  return (activeProblem->upper - activeProblem->lower) * MAX_STEP_FRACTION;
}

static double GetMinimumStep(void) {
  return (activeProblem->upper - activeProblem->lower) * MIN_STEP_FRACTION;
}
...
static void UpdateHQCoverage(const double *position) {
...
static void RegisterTestedPoint(const double *position, double value) {
  ...
  CopyVector(testedPoints[testedPointCount].position, position,
             activeProblem->dim);
...
static double Evaluate(const double *position) {
  double value = activeProblem->function(position, activeProblem->dim);
...
static void MoveAgent(Agent *agent, int direction) {
  LocalPoint *point = &agent->localPoints[direction];

  CopyVector(agent->position, point->position, activeProblem->dim);
...
static void TestDirection(Agent *agent, int direction) {
  LocalPoint *point = &agent->localPoints[direction];
...
static int GetDirection(const Agent *agent, int offset) {
  return (agent->baseDirection + offset) % DIRECTION_COUNT;
}

static int FindBestTestedDirection(const Agent *agent) {
  int bestDirection = -1;
...
static int GetLargestHoleCandidate(double *x, double *y, double *clearance) {
  if (globalCoverageComplete)
    return 0;
...
static void TeleportAgentForExploration(Agent *agent) {
  ...
  if (!GetLargestHoleCandidate(&x, &y, &clearance)) {
...
static void StepAgent(Agent *agent) {
  ...
  if (agent->phase == PHASE_PERPENDICULAR_2) {
    TestDirection(agent, perpendicular2);
...
static void ScoutHQInteractiveInit(const TestProblem *problem) {
...
static int ScoutHQInteractiveGetPointCount(void) {
  return testedPointCount + AGENT_COUNT + AGENT_COUNT * DIRECTION_COUNT;
}

static void ScoutHQInteractiveGetPoint(int index, double *x, double *y,
                                       int *type) {
...
void ScoutHQGetDiagnostics(ScoutHQDiagnostics *diagnostics) {
  double minimumStep = INFINITY;

  for (int i = 0; i < AGENT_COUNT; i++) {
    if (agents[i].stepSize < minimumStep)
      minimumStep = agents[i].stepSize;
  }
